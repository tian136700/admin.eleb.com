...
@section("contents")
    ...
    <form action="{{route("events.store")}}" method="post" class="form-horizontal">
        ...
        <div class="form-group">
            ...
            <input type="datetime-local" name="signup_start" style="width: 500px" class="form-control" value="{{old("signup_start")}}"/>
        </div>
        <label class="col-xs-2 control-label">报名结束时间</label>
        <div class="form-group">

            <input type="datetime-local" name="signup_end" style="width: 500px" class="form-control" value="{{old("signup_end")}}"/>
        </div>
        <label class="col-xs-2 control-label">开奖日期</label>
        <div class="form-group">

            <input type="datetime-local" name="prize_date" style="width: 500px" class="form-control" value="{{old("prize_date")}}"/>
        </div>
        <label class="col-xs-2 control-label">报名人数限制</label>
        <div class="form-group">

            <input type="number" name="signup_num" style="width: 500px" class="form-control" value="{{old("signup_num")}}"/>
        </div>
